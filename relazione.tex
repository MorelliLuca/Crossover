...
\section{Abstract}
In questa esperienza abbiamo analizzato il comportamento di un filtro crossover a tre vie composto da un filtro passa banda in parallelo con un passa basso e un passa alto, in regime sinusoidale. In particolare, abbiamo acquisito i dati relativi alla tensione e alle fasi in funzione della frequenza per ogni filtro, in modo da poter stimare i valori delle frequenze di crossover ($ \nu_{LM}=(487\pm19)\:Hz, \nu_{LH}=(684\pm44)\:Hz $ e $ \nu_{MH}=(1032\pm16)\:Hz $), che non sono risultati in accordo con quelli attesi ($ \nu_{LM}= (549.7 \pm 5.5) \:Hz, \nu_{LH}= (748.4
\pm 7.5 )\:Hz $ e $ \nu_{MH}= (1139\pm 11 )\:Hz $) e della frequenza di risonanza del filtro passa banda ($ \nu_{Ris}=(714.7\pm43)\: Hz$) che è invece risultato compatibile con quello atteso ($ \nu_{Ris}=(682\pm12)\: Hz$).
Studiando gli sfasamenti dei segnali filtrati rispetto alla sorgente abbiamo ottenuto un'ulteriore stima di $ \nu_{LH}=(700\pm16)\:Hz$ e di $ \nu_{0}=(737.4\pm6.0)\: Hz$ una frequenza caratteristica del fitro passa banda. Ulteriori analisi ci hanno consentito di identificare come causa principale delle discrepanze la presenza di una resistenza interna al generatore che impedisce di erogare una tensione costante al variare della frequenza.
...
Utilizzando la legge per le maglie di Kirchoff su ogni ramo del circuito abbiamo ricavato la funzione di trasferimento di ogni filtro, il cui modulo regola la tensione misurata sul ramo e il cui argomento regola la fase.\\
...
\begin{equation}
	\phi_{M}(\nu_{0})=\arctan \left( \frac{1-(2\pi\frac{1}{2\pi\sqrt{L_M C_M}})^{2}L_{M}C_{M}}{ 2\pi\frac{1}{2\pi\sqrt{L_M C_M}} (R_{gen}+R_{L})C_{M}}\right)=\arctan \left( \frac{0}{ 2\pi\frac{1}{2\pi\sqrt{L_M C_M}} (R_{gen}+R_{L})C_{M}}\right)=\arctan (0)=0
\end{equation}
\end{document}
