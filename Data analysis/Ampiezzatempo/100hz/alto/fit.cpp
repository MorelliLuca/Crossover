...
graph->SetTitle((title+";Tempo [s];Tensione [V]").c_str());
...
if(rumBool[0]=='s'||rumBool[0]=='y'){
  std::fstream in;
  TH1F *rumore = new TH1F("rumore","Rumore",1000000,-10,10);
  in.open("rumore.txt");
  double x;
  for(int i{0};i<100;++i)
  {
  ...
  double error{/*rumore->GetStdDev()*/20e-3};
...
*/
TF1 *fitFun = new TF1("fitFun","[0]*sin(x*2*pi*[1] + [2]) + [3]");
...
noise->SetTitle(";Tempo [s];Tensione [V]");
