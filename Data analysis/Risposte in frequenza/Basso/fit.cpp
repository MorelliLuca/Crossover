...
*/
graph->SetTitle("Risposta in frequenza Woofer;Frequenza [Hz];Tensione [V]");
...
//FIT CON GAUSSIANA
TF1 *fitFun = new TF1("fitFun","[0] / sqrt(pow(x*2*pi*[2]/[1],2) + pow(([1]+50)/[1],2) )");
//TF1 *fitFun = new TF1("fitFun","[0]*(1-[3]*TMath::Gaus(x,[4],[5])) / sqrt((x*2*pi*[2]/[1])^2 + pow(([1]+50)/[1],2) )");
...
graph->Fit("fitFun","O","",1500,3000);

int N{graph->GetN()};
std::vector<double> delta;

TF1 *fitRes = graph->GetFunction("fitFun");
fitRes->DrawCopy("SAME");
...
noise->SetTitle(";Frequenza [Hz];Tensione [V]");
